  ...
  return (
    <div className="reports-page">
      ...
      <div className="breadcrumb">
        <span className="breadcrumb-link" onClick={() => onNavigate && onNavigate('dashboard')}>Home</span>
        <span className="breadcrumb-sep">&gt;</span>
        <span className="breadcrumb-link" onClick={() => onNavigate && onNavigate('assessments')}>Assessments</span>
        <span className="breadcrumb-sep">&gt;</span>
          ...
          <thead>
            <tr>
              <th className="th-check">
                <input
                  type="checkbox"
                  checked={selectedRows.length === reportData.length && reportData.length > 0}
                  onChange={toggleAll}
                />
              </th>
          ...
          <tbody>
            {reportData.map((item, idx) => (
              <tr key={idx} className={selectedRows.includes(idx) ? 'row-selected' : ''}>
                ...
                <td>
                  {item.flags > 0 ? (
                    <span className="flags-badge">{item.flags}</span>
                  ) : (
                    <span className="flags-none">-</span>
                  )}
                </td>
                <td>
